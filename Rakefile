...
def install_term_theme
  ...
  message =
    "I've found #{profiles.size} #{
      profiles.size > 1 ? 'profiles' : 'profile'
    } on your iTerm2 configuration, which one would you like to apply the Solarized theme to?"
  profiles << 'All'
    ...
    begin
      profiles <<
        `
          ...
          :Name" ~/Library/Preferences/com.googlecode.iterm2.plist 2>/dev/null
...
def install_files(files, method = :symlink)
  files.each do |f|
    ...
    if file == 'zshrc'
      File.open(target, 'a') { |zshrc| zshrc.puts('for config_file ($HOME/.yadr/zsh/*.zsh) source $config_file') }
    end
...
def list_vim_submodules
  result =
    `git submodule -q foreach 'echo $name"||"\`git remote -v | awk "END{print \\\\\$2}"\`'`.select do |line|
      line =~ /^vim.bundle/
...
def apply_theme_to_iterm_profile_idx(index, color_scheme_path)
  ...
  values.flatten.each do |entry|
    run " /usr/libexec/PlistBuddy -c \"Delete :'New Bookmarks':#{index}:'#{
          entry
        }'\" ~/Library/Preferences/com.googlecode.iterm2.plist "
  end

  run " /usr/libexec/PlistBuddy -c \"Merge '#{color_scheme_path}' :'New Bookmarks':#{
        index
      }\" ~/Library/Preferences/com.googlecode.iterm2.plist "
